...
# -----------------------------------------------------------
# 1. 파일 관리 및 설정 (2025-01-01 시작 기본값)
# -----------------------------------------------------------
SETTINGS_FILE = 'wedaeri_settings_final.json'
LOG_FILE = 'wedaeri_trade_log_final.csv'
PROFIT_FILE = 'wedaeri_profit_log_final.csv'
...
def run_wedaeri_engine(df, start_dt, end_dt, params):
    ...
    for date, row in sim_data.iterrows():
        ...
        # [핵심] 유동적 현금 한도 계산: 현재 총자산을 기준으로 매주 갱신
        current_total_asset = cash + (shares * price)
        dynamic_max_cash = current_total_asset * (params['max_cash_pct'] / 100)
        ...
        if is_first:
            ...
            if diff_val > 0: # 상승 매도
                ...
                # [수정] 현재 가용 가능한 현금 범위 내에서 매수 (유동적 한도 적용)
                available_to_use = dynamic_max_cash - (current_total_asset - cash)
                trade_val = min(cash, trade_val, max(0, available_to_use))
        ...
        logs.append({
            ...
            '종가': round(price, 2), '거래금액': round(trade_val, 0), 
...
with tab1:
    ...
    with st.expander("📋 매매 로그 및 수익 일지 수정"):
        ...
        st.write("---")
        if not os.path.exists(PROFIT_FILE): pd.DataFrame(columns=['Date', 'Total_Asset', 'Return_Pct']).to_csv(PROFIT_FILE, index=False)
...
with tab2:
    ...
    if run_bt:
        ...
        if not res.empty:
            ...
            st.subheader("📋 벡테스트 상세 로그")
            st.dataframe(pd.DataFrame(logs).sort_values('날짜', ascending=False), use_container_width=True)

with tab3:
    st.markdown("### 📘 위대리 v1.1 핵심 메커니즘")
    st.info("💡 본 시스템은 자산 규모가 커짐에 따라 현금 방패(Max Cash Limit)를 자동으로 확장하여 하락장 방어력을 극대화합니다.")
