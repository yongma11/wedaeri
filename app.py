...
settings = load_json(SETTINGS_FILE, {
    'start_date': '2010-02-12', # TQQQ 탄생 시점
...
# -----------------------------------------------------------
# 2. 정밀 매매 엔진 (이전 시뮬레이터 엔진 완벽 이식)
# -----------------------------------------------------------
def calculate_growth_curve(series, window=1260):
    """5년(1260일) 슬라이딩 윈도우 기반 정밀 로그 회귀"""
...
def fetch_complete_data():
    # 1. QQQ는 2000년부터 불러와서 2010년 시점에 이미 '예열'된 추세선을 만듭니다.
    ...
    # 5년 윈도우 추세선 산출 (2000년부터 계산하므로 2010년은 이미 안정화됨)
...
# 4. 메인 화면 레이아웃
# -----------------------------------------------------------
tab1, tab2, tab3 = st.tabs(["🔥 실전 트레이딩", "📊 백테스트 분석", "📘 전략 가이드"])
...
with tab2:
    ...
    if run_bt:
        ...
        if not res.empty:
            ...
            st.pyplot(fig_bt)
            st.subheader("📋 상세 매매 로그"); st.dataframe(pd.DataFrame(logs).sort_values('Date', ascending=False), use_container_width=True)

with tab3:
    st.markdown("""
    <div class="strategy-card">
        <h2>📘 위대리(Wedaeri) v1.1 Pro 전략 상세 가이드</h2>
        <p>본 전략은 나스닥 100 지수(QQQ)의 <b>전체 성장 역사</b>를 기반으로, 3배 레버리지(TQQQ)의 변동성을 수익으로 전환하는 시스템입니다.</p>
        <h3>1. 핵심 메커니즘: 역사적 로그 회귀</h3>
        <p>2000년부터 현재까지의 모든 데이터를 매주 학습하여 시장의 '진짜 적정 가치'를 산출합니다. 이를 통해 일시적 소음에 휘둘리지 않고 복리의 힘을 극대화합니다.</p>
        <h3>2. 유동적 현금 방패</h3>
        <p>자산 규모에 따라 하락장에서 대응 가능한 현금 한도를 자동으로 확장합니다. 자산이 늘어날수록 방패도 커지는 든든한 리스크 관리 시스템입니다.</p>
    </div>
    """, unsafe_allow_html=True)
