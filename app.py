# wedaeri_app.py — TQQQ 위대리 v4.2
# Tab1: 실전 트레이딩 | Tab2: 백테스트 분석 | Tab3: 전략 로직
#
# v4.2 변경사항 (vs v4.0):
#   • F-가드 (변동성 정규화) 통합
#       13주 실현 변동성으로 주문 수량 자동 스케일링.
#       size_factor = clip(0.40 / realized_vol, 0.5, 1.5)
#       — 변동성 폭발 시 자동 감속, 잠잠할 때 가속.
#       — 워크포워드 검증: Calmar 1.71→1.87, 평균 MDD -25%→-20%
#   • 사이드바에 F-가드 on/off 토글
#   • Sheets '설정' 시트에 use_volF 키 저장 (봇과 동기화)
#   • Tab1 에 현재 size factor / 실현 변동성 메트릭 표시
#   • Tab3 에 F-가드 해설 섹션 추가
...
# F-가드 (변동성 정규화) — 봇과 동일 상수
VOL_LOOKBACK   = 13     # 13주
VOL_TARGET     = 0.40   # 연환산 40% 목표 변동성
VOL_FACTOR_MIN = 0.5
VOL_FACTOR_MAX = 1.5
...
def save_config(ss) -> dict:
    ...
    cfg = {
        ...
        'use_volF': bool(ss.get('p_use_volF', DEFAULT_CONFIG['use_volF'])),
    }
...
def save_config_to_sheets(cfg: dict) -> tuple[bool, str]:
    try:
        ...
        return False, str(e)
# ─────────────────────────────────────────────────────────────
# F-가드 헬퍼: 13주 실현 변동성 → size_factor
# ─────────────────────────────────────────────────────────────
def compute_size_factor_at(weekly_tqqq_series, idx, use_volF=True):
    """idx 시점까지의 누적 데이터로 size_factor 계산 (look-ahead 방지).
    - use_volF=False 면 1.0 반환 (가드 비활성)
    - 데이터 부족 (< VOL_LOOKBACK+1 행) 이면 1.0 반환
    """
    if not use_volF or idx < VOL_LOOKBACK:
        return float('nan'), 1.0
    rets = weekly_tqqq_series.iloc[:idx+1].pct_change().dropna().tail(VOL_LOOKBACK)
...
# 1. 페이지 설정 & 스타일
# ─────────────────────────────────────────────────────────────
st.set_page_config(page_title="위대리 Quantum T-Flow v4.2", layout="wide")
st.markdown("""
<style>
...
.tier-low  { background-color: #d4edda; border-left: 4px solid #28a745;
             padding: 12px 16px; border-radius: 6px; margin: 6px 0; }
.f-guard-card { background-color: #ede9fe; border-left: 4px solid #8b5cf6;
                padding: 12px 16px; border-radius: 6px; margin: 6px 0; }
</style>
...
if '_cfg_loaded' not in st.session_state:
    ...
    ss['p_bM']      = _cfg['bM']
    ss['p_bL']      = _cfg['bL']
    ss['p_use_volF'] = bool(_cfg.get('use_volF', True))
...
with st.sidebar:
    ...
    with st.container(border=True):
        ...
        st.markdown("**🛡️ F-가드 (변동성 정규화)**")
        st_use_volF = st.checkbox(
            "활성화 (권장 ✅)", key='p_use_volF',
            help="13주 실현 변동성으로 주문 수량을 자동 조절.\n"
                 "변동성 폭발 시 사이즈 ↓, 잠잠할 때 사이즈 ↑.\n"
                 "백테스트: Calmar 1.71→1.87, MDD -25%→-20%"
        )
...
**F-가드 (변동성 정규화):** {'✅ ON' if _vF_d else '⏸ OFF'}
...
def run_wedaeri_sim(data, start_dt, init_cap, cash_ratio,
    ...
    for i in range(len(wkly)):
        ...
        # F-가드 — i 시점까지의 누적 변동성으로 사이즈 조절
        rv_i, sf_i = compute_size_factor_at(wkly['TQQQ'], i, use_volF=use_volF)
        action, disp_qty = "관망", 0
        if i > 0:
            prev_p = float(wkly.loc[i-1, 'TQQQ'])
            diff   = shares * (p - prev_p)
            if diff > 0:
                qty = int(min(round(diff * sr * sf_i / p), shares))
                if qty > 0:
                    action, disp_qty = "매도", -qty
                    shares -= qty; cash += qty * p
            elif diff < 0:
                qty = int(min(cash, abs(diff) * br * sf_i) / p)
                if qty > 0:
        ...
        logs.append({
            ...
            'size_factor': round(sf_i, 2),
            '실현변동성': f"{rv_i:.0%}" if not np.isnan(rv_i) else "—",
        })
    return pd.DataFrame(logs)
# ─────────────────────────────────────────────────────────────
# 5. 전체 기간 백테스트 (Tab 2) — F-가드 통합
# ─────────────────────────────────────────────────────────────
def run_full_backtest(data, init_cap=20_000, cash_ratio=0.45,
    ...
    eq     = np.empty(N)
    tiers  = []
    sfs    = np.ones(N)   # F-가드 size factor 시계열
    ...
    for i in range(N):
        ...
        tiers.append(tier)
        # F-가드
        _, sf_i = compute_size_factor_at(wkly['TQQQ'], i, use_volF=use_volF)
        sfs[i] = sf_i
        if i == 0:
            continue
        diff = shares * (p - P[i-1])
        if diff > 0:
            sr_t = {'HIGH': sH, 'MID': sM, 'LOW': sL}[tier]
            qty  = int(min(round(diff * sr_t * sf_i / p), shares))
            shares -= qty; cash += qty * p
        elif diff < 0:
            br_t = {'HIGH': bH, 'MID': bM, 'LOW': bL}[tier]
            qty  = int(min(cash, abs(diff) * br_t * sf_i) / p)
            shares += qty; cash -= qty * p
        eq[i] = cash + shares * p
...
log_df = run_wedaeri_sim(
    ...
    use_volF = _ss.get('p_use_volF', True),
)
...
st.title("🚀 TQQQ [위대리] v4.2 : F-가드 통합 트레이딩 시스템")
...
with tab1:
    ...
    c4.metric("F-가드", "✅ ON" if _ss.get('p_use_volF', True) else "⏸ OFF",
              f"size {last['size_factor']:.2f}× / vol {last['실현변동성']}")
    c5.metric("매매 주기",    "금요일 (주간 LOC)")
    ...
    with s_col:
        ...
        st.markdown(
            ...
            f'<h1>{val}</h1></div>',
            unsafe_allow_html=True)
    if _ss.get('p_use_volF', True):
        sf_v = last['size_factor']
        rv_v = last['실현변동성']
        cap_str = ""
        if sf_v >= VOL_FACTOR_MAX - 1e-9:
            cap_str = "  →  💤 변동성 낮음 → 사이즈 최대(1.5×) 적용"
        elif sf_v <= VOL_FACTOR_MIN + 1e-9:
            cap_str = "  →  ⚡ 변동성 폭발 → 사이즈 최소(0.5×)로 자동 감속"
        st.markdown(
            f'<div class="f-guard-card"><b>🛡️ F-가드 활성</b> — '
            f'13주 실현 변동성 <b>{rv_v}</b> (연환산), 목표 40% '
            f'→ 주문 사이즈 <b>{sf_v:.2f}×</b>{cap_str}</div>',
            unsafe_allow_html=True
        )
    st.divider()
...
with tab2:
    ...
    st.caption("TQQQ 상장 첫 주(2010년 2월)부터 현재까지의 전략 시뮬레이션 — F-가드 적용 가능")
    ...
    # F-가드 ON/OFF 비교 옵션
    bt_compare_volF = st.checkbox(
        "F-가드 비교 모드 (ON vs OFF 동시 표시)", value=False,
        help="현재 파라미터로 F-가드 적용 vs 미적용 결과를 동시에 비교합니다."
    )
    bt_use_volF = _ss.get('p_use_volF', True)
    ...
    st.caption(
        ...
        f"| F-가드 {'ON' if bt_use_volF else 'OFF'}"
    )
    # F-가드 비교
    if bt_compare_volF and bt_volF_off is not None:
        st.markdown("### 🛡️ F-가드 ON vs OFF (같은 파라미터)")
        cmp_df = pd.DataFrame({
            '지표':       ['CAGR', 'MDD', 'Calmar', 'Sortino', '최종 자산'],
            'F-가드 ON':  [f"{bt_cur['cagr']:.2%}",     f"{bt_cur['mdd']:.2%}",
                          f"{bt_cur['cal']:.2f}",      f"{bt_cur['sor']:.2f}",
                          f"${bt_cur['final']:,.0f}"],
            'F-가드 OFF': [f"{bt_volF_off['cagr']:.2%}", f"{bt_volF_off['mdd']:.2%}",
                          f"{bt_volF_off['cal']:.2f}",  f"{bt_volF_off['sor']:.2f}",
                          f"${bt_volF_off['final']:,.0f}"],
        })
    ...
    if bt_compare_volF and bt_volF_off is not None:
        fig_eq.add_trace(go.Scatter(
            x=dates, y=bt_volF_off['eq'], name='F-가드 OFF (비교)',
            line=dict(color='#a855f7', width=1.5, dash='dot'), opacity=0.7
        ))
    fig_eq.add_hline(
    ...
    # F-가드 size_factor 시계열 (활성화 시)
    if bt_use_volF and bt_cur.get('size_factors') is not None:
        st.markdown("### 🛡️ F-가드 size factor 시계열")
        fig_sf = go.Figure()
        fig_sf.add_trace(go.Scatter(
            x=dates, y=bt_cur['size_factors'],
            line=dict(color='#a855f7', width=1.5),
            fill='tozeroy', fillcolor='rgba(168,85,247,0.15)',
            name='size factor',
        ))
        fig_sf.add_hline(y=1.0, line_color='#475569', line_dash='dot',
                          annotation_text='기준 1.0×',
                          annotation_font=dict(color='#94a3b8', size=10))
        fig_sf.add_hline(y=VOL_FACTOR_MAX, line_color='#22c55e', line_dash='dash',
                          annotation_text=f'cap {VOL_FACTOR_MAX}×',
                          annotation_font=dict(color='#22c55e', size=10))
        fig_sf.add_hline(y=VOL_FACTOR_MIN, line_color='#ef4444', line_dash='dash',
                          annotation_text=f'floor {VOL_FACTOR_MIN}×',
                          annotation_font=dict(color='#ef4444', size=10))
        fig_sf.update_layout(
            title=f'주문 사이즈 자동 조절 (목표 변동성 {VOL_TARGET:.0%}/년)',
            yaxis_title='size factor',
            height=240, showlegend=False, **CHART_LAYOUT
        )
        apply_grid(fig_sf)
        st.plotly_chart(fig_sf, use_container_width=True)
        sf_vals = bt_cur['size_factors']
        cap_pct = float((sf_vals >= VOL_FACTOR_MAX - 1e-9).mean())
        flr_pct = float((sf_vals <= VOL_FACTOR_MIN + 1e-9).mean())
        st.caption(
            f"평균 size factor: {sf_vals.mean():.2f}× | "
            f"cap (1.5×) 도달 비율 {cap_pct:.1%} | "
            f"floor (0.5×) 도달 비율 {flr_pct:.1%}"
        )
...
# ═══════════════════════════════════════════════════════════════
with tab3:
    st.markdown("## 📘 위대리 v4.2 전략 완전 해설")
    st.caption("TQQQ Quantum T-Flow — QQQ 추세 기반 3-티어 + F-가드 자동 리밸런싱 시스템")
    st.markdown("""
### 🎯 전략 개요
**위대리 전략**은 나스닥 100(QQQ)의 5년 장기 추세와 현재 가격의 괴리를 측정해,
시장이 **과열(HIGH)** 이면 수익을 실현하고, **침체(LOW)** 이면 공격적으로 매수하는
**역추세 + 추세추종 혼합 리밸런싱 전략**입니다. v4.2 부터는 **F-가드(변동성 정규화)** 가
시장 변동성에 따라 주문 사이즈를 자동 조절합니다.
> 💡 핵심 철학: *"시장 온도계(Eval)가 낮을수록 더 많이 사고, 높을수록 더 많이 판다.
>     단, 변동성이 폭발할 땐 사이즈를 줄이고, 잠잠할 땐 늘린다."*
...
""", unsafe_allow_html=True)
    st.divider()
    # ── F-가드 해설 (NEW v4.2) ───────────────────────────────
    st.markdown("### 🛡️ Step 3. F-가드 — 변동성 정규화 (v4.2 신규)")
    fg_col1, fg_col2 = st.columns([1.4, 1])
    with fg_col1:
        st.markdown(f"""
**F-가드**는 13주 실현 변동성을 측정해 주문 수량을 자동 스케일링합니다.

```python
realized_vol = TQQQ 13주 주간수익률 표준편차 × √52   # 연환산
size_factor  = clip(0.40 / realized_vol, 0.5, 1.5)
order_qty    *= size_factor   # 매수·매도 모두 적용
```

| 시장 상태 | 실현 변동성 | size factor | 의미 |
|-----------|-----------|-------------|------|
| 잠잠 | ~30% | **1.33×** | 사이즈 33% 증가 |
| 보통 | ~40% | **1.00×** | 변경 없음 |
| 격동 | ~60% | **0.67×** | 사이즈 33% 감소 |
| 폭발 | ~80% | **0.50×** | 최소 사이즈 (cap) |

**왜 변동성 정규화인가?**
- TQQQ 변동성은 평시 30% ~ 위기 80%+ 까지 변동
- 같은 hc/lc 임계값과 매수/매도 배율이라도 변동성 폭발 시 신호의 *기대 손익*이 폭증
- 변동성으로 사이즈를 정규화하면 **모든 시기에서 위험 노출이 일정**

**워크포워드 검증 결과 (15개 OOS 폴드):**
- Calmar 1.71 → **1.87** (+9% 개선)
- Sortino 2.36 → **2.50** (+6% 개선)
- 평균 MDD -25% → **-20%**
- CAGR는 31% → 29% 로 약 2%p 감소 (위험-수익 트레이드오프)

→ **위험조정 수익률은 명확히 개선**되며, 가장 안전한 robust 가드입니다.
""")
    with fg_col2:
        # 현재 size factor 카드
        if not log_df.empty:
            cur_sf = float(log_df.iloc[-1]['size_factor'])
            cur_rv = log_df.iloc[-1]['실현변동성']
            sf_color = '#22c55e' if cur_sf >= 1.0 else '#ef4444'
            sf_bg    = 'rgba(34,197,94,0.15)' if cur_sf >= 1.0 else 'rgba(239,68,68,0.15)'
            sf_msg = ('💤 잠잠 — 사이즈 증가 모드' if cur_sf >= 1.4
                      else '⚡ 격동 — 사이즈 감소 모드' if cur_sf <= 0.6
                      else '⚖️ 평상시 — 거의 정상 사이즈')
            st.markdown(f"""
<div style="background:{sf_bg}; border:2px solid {sf_color}; border-radius:14px;
            padding:24px; text-align:center;">
  <p style="color:#94a3b8; margin:0; font-size:14px;">🛡️ 현재 F-가드 상태</p>
  <h1 style="color:{sf_color}; margin:8px 0; font-size:3rem;">{cur_sf:.2f}×</h1>
  <p style="color:white; margin:0;">실현 변동성: <b>{cur_rv}</b> (연환산)</p>
  <p style="color:#94a3b8; margin-top:10px;">{sf_msg}</p>
</div>
""", unsafe_allow_html=True)
    st.divider()
    st.markdown("### ⚙️ Step 4. 매매 실행 로직")
    st.markdown("**매주 금요일 장 마감 10분 전(오후 3:50) LOC 주문으로 실행합니다.**")
    st.code("""
매주 금요일 장 마감 시:
이번 주 TQQQ가 전주 대비 상승했다면:
    평가 수익   = 보유수량 × (이번 주 가격 - 전주 가격)
    매도 수량   = int(평가 수익 × 매도 배율[티어] × size_factor / 현재가)
    실제 매도   = min(매도 수량, 현재 보유수량)   ← 공매도 방지
    → LOC 매도 주문 실행 → 현금 증가

이번 주 TQQQ가 전주 대비 하락했다면:
    평가 손실   = 보유수량 × (전주 가격 - 이번 주 가격)
    매수 예산   = min(보유 현금, 평가 손실 × 매수 배율[티어] × size_factor)
    매수 수량   = int(매수 예산 / 현재가)
    → LOC 매수 주문 실행 → 주식 증가
...
* size_factor = clip(0.40 / 13주 실현 변동성, 0.5, 1.5)   ← F-가드 적용
""", language='python')
    col_up, col_dn = st.columns(2)
    with col_up:
        st.info(f"""
**📈 상승 시 매도 — 차익 실현**
평가 수익의 일정 배율 × size factor 만큼 매도해 현금을 확보합니다.
- **HIGH 티어** → {_sH_d}× : 적극 실현 ★
- **MID 티어** → {_sM_d}× : 소폭 실현
- **LOW 티어** → {_sL_d}× : 최소 실현
→ 과열 구간에서 더 많이 팔아 현금을 쌓아둡니다.
→ 변동성이 높으면 size factor 가 작아져 사이즈가 자동 감속됩니다.
""")
    with col_dn:
        st.success(f"""
**📉 하락 시 매수 — 저점 매집**
평가 손실의 배율 × size factor 만큼 현금을 투입해 추가 매수합니다.
- **HIGH 티어** → {_bH_d}× : 손실의 일부만 매수
- **MID 티어** → {_bM_d}× : 균형 매수
- **LOW 티어** → {_bL_d}× : 공격적 매수 ★
→ 시장이 차가울수록 더 많이 사서 평균 단가를 낮춥니다.
→ 변동성 폭발 시 size factor 가 자동으로 줄여 leverage 누적을 방지합니다.
""")
    st.divider()
...
- 봇은 금요일 ET 04:00–09:30 (pre-market) 또는
  16:05 이후 (post-close) 실행
- 시장 휴장 시 전날 대체
""")
    with tip2:
        st.info("""
...
- 워크포워드는 디폴트가 robust 함을 증명
- 잦은 파라미터 변경 ✗
- 워크포워드 OOS MDD 최악 -71% 가능성 인지
""")
    st.divider()
    st.markdown("""
### 📊 위대리 v4.2 vs v4.0 vs TQQQ B&H 핵심 비교
| 항목 | v4.2 (F-가드 ON) | v4.0 (가드 OFF) | TQQQ B&H |
|------|:---:|:---:|:---:|
| **연평균 수익(CAGR)** | ~29% | ~31% | ~45% |
| **평균 OOS MDD** | **~20%** | ~25% | ~48% |
| **Calmar 비율** | **1.87** | 1.71 | ~0.93 |
| **Sortino 비율** | **2.50** | 2.36 | ~1.20 |
| **심리적 안정성** | ★★★★★ | ★★★★ | ★★ |
| **주간 주문 횟수** | 1회 | 1회 | 없음 |

> **F-가드 v4.2 의 핵심 가치**: 연 수익을 약간 양보하는 대신 변동성 폭발기에 자동 감속.
> Calmar/Sortino 모두 명확히 개선되어 **장기 운용에 가장 robust**합니다.
> 워크포워드 OOS 검증 완료. 사용자 디폴트 파라미터와 잘 맞물립니다.
""")
