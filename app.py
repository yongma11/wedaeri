...
with tab3:
    ...
    <div class="strategy-card">
        <h2 style="color:#0D47A1; border-bottom:2px solid #0D47A1; padding-bottom:10px;">📘 Wedaeri Quantum T-Flow 전략 가이드</h2>
        
        <p>본 전략은 나스닥100 3배 레버리지(TQQQ)의 <b>'극단적 변동성'</b>을 인위적인 리밸런싱을 통해 <b>'복리 수익'</b>으로 치환하는 퀀트 시스템입니다.</p>

        <h3 style="color:#1E88E5;">1. 핵심 로직: 로그 회귀 이격도 (Log-Regression Eval)</h3>
        <p>시장의 소음(Noise)을 제거하기 위해 QQQ의 지난 25년 데이터에 로그 회귀(Log-Regression)를 적용합니다. 
        이를 통해 산출된 <b>'역사적 평균 성장선(Growth Line)'</b> 대비 현재 가격의 이격도를 측정하여 시장의 과열(HIGH)과 침체(LOW)를 정량적으로 판별합니다.</p>

        <h3 style="color:#1E88E5;">2. 주간 변동성 리밸런싱 (Weekly Rebalancing)</h3>
        <p>매일 매매할 경우 수수료와 슬리피지가 수익을 갉아먹습니다. 본 시스템은 <b>매주 금요일 종가</b>만을 확인합니다. 
        한 주 동안 발생한 자산 가치의 변화분(Volatility)에 티어별 가중치를 곱하여 기계적으로 수량을 조절합니다.</p>
        <ul>
            <li><b>상승 시 (익절):</b> 올라간 가치만큼 정해진 비율로 매도하여 현금을 확보합니다.</li>
            <li><b>하락 시 (추매):</b> 내려간 가치만큼 확보된 현금으로 추가 매수하여 평단가를 낮추고 수량을 늘립니다.</li>
        </ul>

        <h3 style="color:#1E88E5;">3. 전략의 3대 장점</h3>
        <ol>
            <li><b>MDD 방어:</b> 하락장에서 '현금 방패' 로직이 작동하여 TQQQ의 -90% 폭락 구간에서도 생존을 보장합니다.</li>
            <li><b>수량 극대화:</b> 횡보장과 하락장에서 기계적 추매를 통해 상승장이 왔을 때 폭발적인 수익을 낼 수 있는 수량을 확보합니다.</li>
            <li><b>심리적 안정:</b> 감정을 배제하고 '오늘 주문표'에 나온 수치대로만 매매하므로 뇌동매매를 원천 차단합니다.</li>
        </ol>

        <h3 style="color:#1E88E5;">4. 실전 매매 가이드</h3>
        <p>금요일 밤(미국시간), 대시보드의 <b>'오늘 주문표'</b>에 예상 종가를 입력하세요. 
        시스템이 계산해주는 정수 수량만큼 증권사 앱에서 주문을 넣는 것으로 한 주의 매매는 끝납니다. 남은 시간은 본업과 휴식에 집중하세요.</p>
    </div>
    """, unsafe_allow_html=True)
