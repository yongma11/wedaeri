...
def fetch_data_reconstructed():
    # [핵심 1] auto_adjust=False를 사용하여 이전 시뮬레이터와 동일한 '비배당 조정' 가격 사용
    qqq_raw = yf.download("QQQ", start="2000-01-01", progress=False, auto_adjust=False)
    tqqq_raw = yf.download("TQQQ", start="2010-01-01", progress=False, auto_adjust=False)
    
    # [핵심 2] Split-adjusted Close 가격 사용 (배당으로 인한 왜곡 방지)
    qqq = qqq_raw['Close'].squeeze()
    tqqq = tqqq_raw['Close'].squeeze()

    # QQQ 역사적 추세선 선계산 (2000년부터 예열)
...
def run_wedaeri_engine_v11(df, start_dt, end_dt, params):
    ...
    for date, row in sim_data.iterrows():
        ...
        # [핵심 3] 유동적 자본 스케일링: 현재 총자산 대비 현금 한도 리밸런싱
        dynamic_max_cash = total_asset * (params['max_cash_pct'] / 100)
        
        # 티어별 매매 비율 (용성님 최적 파라미터)
        ...
        if is_first:
            ...
            shares = round(trade_val / price, 2); cash -= (shares * price)
            ...
            if diff_val > 0: # 상승 매도
                ...
                qty_to_sell = round(trade_val / price, 2)
                actual_sell_qty = min(qty_to_sell, shares)
                shares -= actual_sell_qty; cash += (actual_sell_qty * price); action = "Sell"
                trade_val = actual_sell_qty * price
            elif diff_val < 0: # 하락 매수
                trade_val = abs(diff_val) * b_r
                # 유동적 한도 내 가용 현금 체크
                avail_limit = dynamic_max_cash - (total_asset - cash)
                actual_buy_val = min(cash, trade_val, max(0, avail_limit))
                qty_to_buy = round(actual_buy_val / price, 2)
                shares += qty_to_buy; cash -= (qty_to_buy * price); action = "Buy"
                trade_val = qty_to_buy * price
...
# 3. 사이드바 및 동기화
# -----------------------------------------------------------
df_weekly = fetch_data_reconstructed()
...
if sync_btn:
    ...
    # 실전 로그 생성
    _, res_logs = run_wedaeri_engine_v11(df_weekly, set_date, datetime.now(), settings)
    if res_logs:
...
with tab1:
    ...
    with c1: st.markdown(f'<div class="sub-text">시장모드</div><div class="big-metric" style="color:{m_col};">{m_tier} ({eval_val*100:.1f}%)</div>', unsafe_allow_html=True)
    ...
    with st.expander("📋 매매 로그 및 수익 일지 기록 (수정 가능)", expanded=False):
        st.data_editor(t_log, num_rows="dynamic", use_container_width=True, key="re_log")
        st.write("---")
        ed_p = st.data_editor(p_log, num_rows="dynamic", use_container_width=True, key="re_prof")
        if not ed_p.empty:
            p_plot = ed_p.copy(); p_plot['Date'] = pd.to_datetime(p_plot['Date']).sort_values()
            fig_r, ax_r1 = plt.subplots(figsize=(12, 4))
            ax_r1.plot(p_plot['Date'], p_plot['Total_Asset'], color='#1E88E5', marker='o', lw=2)
            ax_r1.set_title("실전 자산 성장 곡선"); ax_r1.grid(True, alpha=0.2)
...
with tab2:
    ...
    if run_bt:
        res, logs = run_wedaeri_engine_v11(df_weekly, bt_start, bt_end, {'initial_capital': bt_cap, 'max_cash_pct': settings['max_cash_pct'], 'initial_entry_pct': settings['initial_entry_pct']})
        if not res.empty:
            ...
            m5.metric("Calmar", f"{calmar:.2f}"); m6.metric("샤프 지수", f"{sharpe:.2f}"); m7.metric("소르티노 지수", f"{sortino:.2f}"); m8.metric("최종 자산", f"${final_v:,.0f}")
...
with tab3:
    ...
    <div class="strategy-card">
        ...
        <p>본 전략은 나스닥 100 지수(QQQ)의 <b>25년 장기 성장 궤적</b>을 기반으로, 3배 레버리지(TQQQ)의 변동성을 수익으로 전환하는 정량적 리밸런싱 시스템입니다.</p>
        
        <h3>1. 핵심 메커니즘: 역사적 로그 회귀 (Historical Regression)</h3>
        <p>기존의 짧은 5년 데이터는 2008년 같은 대폭락의 역사를 망각합니다. 본 시스템은 2000년부터 현재까지의 모든 데이터를 누적 학습하여 시장의 '진짜 적정 가치'를 산출합니다. 이를 통해 일시적 소음에 휘둘리지 않고 복리의 힘을 극대화합니다.</p>
        
        <h3>2. 유동적 현금 방패 (Dynamic Cash Shield)</h3>
        <p>자산 규모에 따라 하락장에서 대응 가능한 현금 한도를 자동으로 확장합니다. 자산이 늘어날수록 방패도 커지는 든든한 리스크 관리 시스템으로 MDD를 30% 수준으로 철저히 방어합니다.</p>
