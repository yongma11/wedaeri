...
# ─────────────────────────────────────────────────────────────
# 주문표 신선도 스탬프  (동파공 bot.py v10.2.12 와 같은 규격)
#
#   봇이 안 도는 주에도 시트에는 '지난주 표'가 그대로 남는다. 자동주문
#   (autotrade/trader_gui.py)은 그게 이번 주 것인지 알 방법이 없어 같은
#   주문을 다시 낸다. 동파공에서 실제로 터진 사고다(2026-08-27, GitHub
#   Actions 미실행 → 이미 체결된 매수 621주 재발주).
#   막을 수 있는 건 소비자가 낡은 표를 '구별하게' 해 주는 것뿐이다.
#
#     A9/B9   봇실행시각(KST)  이 표를 언제 만들었나  → 경과 시간으로 판정
#     A10/B10 데이터기준일      어느 일봉까지 썼나     → 시세 지연 판정
#
#   동파공은 A7 에 예상보유수량도 남기지만 위대리는 남기지 않는다.
#   이 계좌는 사람이 중간에 임의로 사고 팔아 조절하므로, 봇이 추정한
#   보유수량과 실계좌가 어긋나는 것이 정상이다. 대조 근거가 될 수 없는
#   값을 적어두면 자동주문이 멀쩡한 표를 낡은 표로 오해한다.
#
#   주문 행(L4:O4)을 먼저 쓰고 스탬프를 나중에 찍는다. 스탬프만 실패하면
#   '새 표 + 낡은 스탬프'가 남아 소비자가 멈춘다 — 주문이 안 나가는 쪽으로
#   실패한다. 그게 안전한 방향이다.
# ─────────────────────────────────────────────────────────────
STAMP_TIME_RANGE = "A9:B10"      # 봇실행시각(KST) / 데이터기준일
INITIAL_CAP   = 108_000
INIT_CASH_PCT = 0.45
...
# 5. 메인
# ─────────────────────────────────────────────────────────────
def write_freshness_stamp(ws, data_last) -> None:
    """주문표가 '언제·어느 데이터로' 만들어졌는지 시트에 남긴다.

    자동주문 쪽이 낡은 표를 그대로 내보내는 사고를 막기 위한 근거다.
    주문 행을 먼저 쓰고 이 함수를 나중에 부른다.

    스탬프 기록이 실패해도 여기서 죽이지 않는다. 주문표는 이미 들어갔고,
    낡은 스탬프가 남으면 소비자가 알아서 멈춘다.
    """
    try:
        now_kst = datetime.now(ZoneInfo("Asia/Seoul")).strftime('%Y-%m-%d %H:%M:%S')
        ws.batch_update([
            {'range': STAMP_TIME_RANGE,
             'values': [["봇실행시각(KST)", now_kst],
                        ["데이터기준일", str(data_last)]]},
        ])
        print(f"✅ 신선도 스탬프 기록 — {now_kst} 생성 / 데이터기준일 {data_last}")
    except Exception as e:
        # 여기서 죽이지 않는다. 주문표는 이미 들어갔다.
        #
        # 다만 조용히 넘기면 안 된다. 스탬프가 안 찍히면 자동주문 쪽은
        # '검사할 근거가 없다'며 검사를 건너뛴다(skip). 즉 낡은 표 차단이
        # 통째로 꺼진 채로 주문이 나간다. 그 사실을 사람이 알아야 한다.
        print(f"⚠️ 신선도 스탬프 기록 실패: {e}")
        send_telegram(
            "⚠️ *[위대리-H]* 신선도 스탬프 기록 실패\n\n"
            f"`{type(e).__name__}: {str(e)[:200]}`\n\n"
            "주문표(L4:O4)는 정상 기록됐습니다. 다만 A9:B10 스탬프가 없어 "
            "자동주문의 *낡은 표 차단이 꺼진 상태*로 실행됩니다.\n"
            "시트 A9:B10 을 확인하세요."
        )
...
def main():
    ...
    try:
        ...
        if not is_test_mode:
            ...
            # 주문 행을 다 쓴 뒤에 찍는다. 순서가 바뀌면 안 된다.
            # df 의 마지막 행이 이 표가 쓴 마지막 일봉이다.
            #   pre_open/test  -> 직전 완결 세션(목요일 종가)
            #   near_close/post_close -> 오늘(금요일) 행을 붙이거나 덮어썼다
            write_freshness_stamp(
                ws, pd.Timestamp(df['Date'].iloc[-1]).strftime('%Y-%m-%d'))
        else:
